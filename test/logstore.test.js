...
const dbPath = './orbitdb/tests/logstore'
const ipfsPath = './orbitdb/tests/logstore/ipfs'

Object.keys(testAPIs).forEach(API => {
  describe(`orbit-db - 232c Log Database (${API})`, function() {
    ...
    describe('Logstore', function () {
      it('creates and opens a database', async () => {
        db = await orbitdb1.logstore('log database', {
          meta: 'test'
        })
        ...
        assert.equal(db.type, '232c:logstore')
        assert.equal(db.dbname, 'log database')
      })

      it('returns the db metadata', async () => {
      ...
      it('returns the added entry\'s hash, 1 entry', async () => {
        db = await orbitdb1.logstore('first database')
      ...
      it('adds five items', async () => {
        db = await orbitdb1.logstore('second database')
        await mapSeries([1, 2, 3, 4, 5], (i) => db.add('hello' + i))
        const items = db.iterator({ limit: -1 }).collect()
        assert.equal(items.length, 5)
        assert.equal(items[0].payload, 'hello1')
        assert.equal(last(items.map((f) => f.payload)), 'hello5')
      })

      it('adds an item that is > 256 bytes', async () => {
        db = await orbitdb1.logstore('third database')
    ...
    describe('Iterator', function() {
      ...
      before(async () => {
        hashes = []
        db = await orbitdb1.logstore('iterator tests')
      ...
      describe('Defaults', function() {
        ...
        it('returns an item with the correct structure', () => {
          ...
          assert.equal(next.payload, 'hello4')
        })
        ...
        it('returns 1 item as default', () => {
          ...
          assert.equal(first.payload, 'hello4')
        })
        ...
        it('returns items in the correct order', () => {
          ...
          for(let item of iter) {
            assert.equal(item.payload, 'hello' + i)
            i ++
          }
        })
      })
      ...
      describe('Collect', function() {
        it('returns all items', () => {
          ...
          assert.equal(messages.length, hashes.length)
          assert.equal(messages[0].payload, 'hello0')
          assert.equal(messages[messages.length - 1].payload, 'hello4')
        })
